...
const UsersSignUp: React.FC = () => {
  ...
  const handleSignUp = async () => {
    ...
    try {
      const response = await axios.post('https://kosh-marketplace-backend.onrender.com/api/v1/users/signup', { email, password });
  ...
  return (
    <div className="d-flex flex-column min-vh-100" style={{ fontFamily: 'Arial, sans-serif' }}>
      <div className="d-flex flex-column justify-content-center align-items-center"
        ...
        <p className="text-white mb-0" style={{ fontSize: isSmallScreen ? '16px' : '20px' }}>Create your account</p>
      </div>

      <div className="flex-grow-1 d-flex flex-column justify-content-start align-items-center p-3" style={{ background: '#fff' }}>
        <h2 className="fw-bold mb-4" style={{ fontSize: isSmallScreen ? '24px' : '32px', color: '#000F5A', marginTop: isSmallScreen ? '20px' : '40px' }}>Sign up</h2>
        ...
        <div className="d-flex flex-column align-items-center text-dark pb-4" style={{ width: isSmallScreen ? '280px' : '320px' }}>
          ...
          <button onClick={handleSignUp} className="btn btn-primary d-flex justify-content-center align-items-center gap-2 mt-3 w-100"
          ...
          <div className="text-center mt-3" style={{ fontSize: isSmallScreen ? '14px' : '16px', color: '#000F5A' }}>
            Already have an account? <span style={{ textDecoration: 'underline', cursor: 'pointer', color: '#000F5A', fontWeight: 'bold' }}
